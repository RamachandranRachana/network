...
pathaj=shortestpathAnnChan(graph,Ann.name,Jan.name)
pathja=[]
for i in range(len(pathaj)-1,-1,-1):
    pathja.append(pathaj[i])
#connectionSetup(Chan.name,Ann.name,pathca)


#Starting the client socket for chan
pack={'src': 'Jan','dest':'Ann','i':1,'syn':1,'ack':0,'seqno':0,'ackno':0,'checksum':10001010,'drp':0,'ter':0,'rst':0,'fin':0,'urg':0}
j=pack['i']
c=pathja[j]
...
if c=='L':
    host=L.ip
    port=L.port
print('Jan is up and running')
print ('Jan is establishing a communicatin with Ann')
#Start commununication from Jan to Ann
Jan.createClientSocket(host,port,pack)
Jan.startServerListener()
